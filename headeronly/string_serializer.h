...
//WP::oSerializer dient z.zt. nur dem anhängen von werten als zeichenkette an einen string_typ 
//per operator<< 
//
//using string_type = CString;// oder CStringA std::string std::wstring
//usage 
//string_type str = WP::oSerializer<string_type>() << value1 << value2 << valueN;
//oder
//string_type str;
//WP::oSerializer<string_type>(str) << value1 << value2 << valueN;
...
namespace WP
{
	...
	{
		static_assert(WP::is_string_type<senke_t>::value, "senke_t muss z.zt. ein StringType sein." );
...
template<typename senke_t> WP::oSerializer<senke_t> const & operator<<( WP::oSerializer<senke_t> const & o, senke_t const & r )
{
	o.senke += r;
	return o;
}
template<typename senke_t> WP::oSerializer<senke_t> const & operator<<( WP::oSerializer<senke_t> const & o, wchar_t const * r )
{
	return o << senke_t( r );
}
template<typename senke_t> WP::oSerializer<senke_t> const & operator<<( WP::oSerializer<senke_t> const & o, char const * r )
{
	return o << senke_t( r );
}

template<typename senke_t> WP::oSerializer<senke_t> const & operator<<( WP::oSerializer<senke_t> const & o, int r )
{
