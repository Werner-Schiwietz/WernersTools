...
namespace WS
{
	...
	template<typename return_type, typename ... parameter_types,typename combiner_type>struct Signal<return_type(parameter_types...),typename combiner_type> : Signal_trait<return_type(parameter_types...)>
	{
			...
			if constexpr(std::is_same<return_t,void>::value==false)
			{
				...
				for(auto &[prio,callbacks] : this->prio_callbacks)
					for(auto &[id,fn] : callbacks)
						combiner( fn(args...), id_type{id,prio} );
				//return_value = fn(std::forward<parameter_types>(args)...);
				return combiner;
			}
			else for(auto &[prio,map] : this->prio_callbacks)
				for(auto &[id,fn] : map)
					fn(args...);
		}
		...
		std::map<typename id_type::prio_t,std::map<typename id_type::id_t,fn_t>> prio_callbacks;//Funktionspointer-Verwaltung
	private:
