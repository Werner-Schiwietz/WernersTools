...
*	 validkey_type factory-type ergänzt den key um bool valid() (standard WS::Pure_key key immer valid)
* 
* Cache::Cache( ) 
*  -- ctor eines Cache
* 
...
* -- setzt key-value-pair. der rückgabewert sollte ignoriert werden. es ist ein lock_guard auf den mutex und ein iterator auf das verwaltete pair
* 
...
- -- wie CacheDuration, aber jeder Get() verlängert die valid-zeit


//struct WS::null_mutex für singlethreaded nichts tuender mutex

* valid-keys ergänzen den id-key um valid-informationen
* template<typename key_type>
* struct WS::Pure_key -- der value zum key vom type key_type ist immer gültig
* 
* template<typename key_type,int duration_value,typename duration_type=std::chrono::milliseconds>
* struct WS::Duration_key   -- der value zum key vom type key_type bleibt duration_type{duration_value} gültig. also eine bestimmt zeit nach WS::Cache::Set
* 
* template<typename key_type,int duration_value,typename duration_type=std::chrono::milliseconds>
* struct DurationRefresh_key -- wie WS::Duration_key, aber valid-zeit wird mit jedem WS::Cache::Get auf now + duration_type{duration_value} verlängert
*/
...
namespace WS
{
	...
	//_validkey_maker template-factory um aus einem key einen key mit valid-prüfung zu machen
